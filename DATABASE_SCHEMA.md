...
| `_document_history` | `text` | Provenance line from the source document's `Document History` section (approval and revision dates). Written by extraction when the document uses the labelled schema. |
...
`text` is always plain text — search indexing and AI context (`aidocs_blocks_plain_text()`) never need to read `runs`. `runs` (`[ ['text'=>…,'b'=>1,'i'=>1], … ]`) only appears when the source carried bold or italic emphasis. List `style` is one of `decimal`, `lower-alpha`, `upper-alpha`, `lower-roman`, `upper-roman`, `bullet`.
